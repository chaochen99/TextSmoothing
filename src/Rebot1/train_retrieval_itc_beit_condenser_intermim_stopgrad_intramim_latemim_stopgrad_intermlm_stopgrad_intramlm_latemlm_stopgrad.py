...
from models.blip_retrieval_itc_beit_condenser_intermim_stopgrad_intramim_latemim_stopgrad_intermlm_stopgrad_intramlm_latemlm_stopgrad import blip_retrieval
import utils
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    for i,(image4encoder, image4vqkd, bool_masked_pos, caption, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_intramim, loss_intramlm, loss_intermim, loss_intermlm, loss_latemim, loss_latemlm = model(image4encoder, image4vqkd, bool_masked_pos, caption, alpha = alpha, idx=idx)  
...
def main(args, config):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset('retrieval_%s'%config['dataset'], config, min_scale=0.2, num_mask_patches=args.mim_mask_patches)  
    ...
    model = blip_retrieval(pretrained=args.pretrained, image_size=config['image_size'], vit=config['vit'], queue_size=config['queue_size'],
                              mim_early_layers=args.mim_early_layers, mim_cls_layers=args.mim_cls_layers, mim_tie=args.mim_tie, 
                              mlm_early_layers=args.mlm_early_layers, mlm_cls_layers=args.mlm_cls_layers, mlm_probability=args.mlm_probability, mlm_tie=args.mlm_tie, )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_two_tower', default=0, type=int)
