...
from data_beit.coco_karpathy_dataset import coco_karpathy_train, coco_karpathy_caption_eval, coco_karpathy_retrieval_eval
from data_beit.nocaps_dataset import nocaps_eval
from data_beit.flickr30k_dataset import flickr30k_train, flickr30k_retrieval_eval
from data_beit.vqa_dataset import vqa_dataset
from data_beit.nlvr_dataset import nlvr_dataset
from data_beit.pretrain_dataset import pretrain_dataset
from transform.randaugment import RandomAugment
from models.beitv2 import DataAugmentationForBEiT

def create_dataset(dataset, config, min_scale=0.5, num_mask_patches=75, window_size=14, min_mask_patches_per_block=16):
    
    normalize = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))

    transform_train = transforms.Compose([        
            DataAugmentationForBEiT(window_size=window_size, num_mask_patches=num_mask_patches, max_mask_patches_per_block=None, min_mask_patches_per_block=min_mask_patches_per_block, min_scale=0.5),                
            # transforms.RandomResizedCrop(config['image_size'],scale=(min_scale, 1.0),interpolation=InterpolationMode.BICUBIC),
            # transforms.RandomHorizontalFlip(),
            # RandomAugment(2,5,isPIL=True,augs=['Identity','AutoContrast','Brightness','Sharpness','Equalize',
            #                                   'ShearX', 'ShearY', 'TranslateX', 'TranslateY', 'Rotate']),   
            # DataAugmentationForBEiT(window_size=14, num_mask_patches=75, max_mask_patches_per_block=None, min_mask_patches_per_block=16,),
            # transforms.ToTensor(),
            # normalize,
        ])        
