...
import json
import os
from data.data_video.utils import pre_caption
# from video_dataset.DiDeMo import decoder
import lmdb
import io
import av 
from torchvision import transforms
from data.data_video.data_utils import (
...
class msrvtt7k_dataset(Dataset):
    ...
    def __init__(self, video_root,data_split, num_frm=4, frm_sampling_strategy="rand", max_img_size=384, video_fmt='.mp4'):
        ...
        if data_split=='test':
            json_file = '/nlp_group/wuxing/gaochaochen/VLP/RaP/meta_data/MSRVTT_7k/msrvtt_test.jsonl'
        else:
            json_file = os.path.join("/nlp_group/wuxing/gaochaochen/VLP/RaP/meta_data/MSRVTT_7k/txt",f'{data_split}_videodatainfo.json')
...
class msrvtt9k_compact_dataset(Dataset):
    ...
    def __init__(self, video_root,data_split, num_frm=4, frm_sampling_strategy="rand", max_img_size=384, video_fmt='.mp4'):
        self.video_root = video_root
        json_fp = f'/nlp_group/wuxing/gaochaochen/VLP/RaP/meta_data/MSRVTT_9k/annotation/MSR_VTT.json'
        with open(json_fp, 'r') as fid:
            data = json.load(fid)
        df = pd.DataFrame(data['annotations'])
        split_dir='/nlp_group/wuxing/gaochaochen/VLP/RaP/meta_data/MSRVTT_9k/high-quality/structured-symlinks'
